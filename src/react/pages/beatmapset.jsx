...
export default function BeatmapSet({ id }) {
    ...
    useEffect(() => {
        const fetchBeatmapset = async () => {
            ...
            try {
                if (!/^\d+$/.test(id)) {
                    setError('Invalid beatmapset ID');
                    return;
                }

                const res = await fetch(`/api/beatmapset/${id}`);

                if (!res.ok) {
                    throw new Error('Failed to fetch beatmapset');
                }
